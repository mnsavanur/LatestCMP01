...
public class Regression_Scenario_GL_009
        ...
        implements CMPLogin_Page.ILogin, CMPHome_Page.IHome, CommonInfo_Page.ICommonInfo, Names_Page.INames, Rating_Page.IRating, Issue_Page.IIssue_Page, IssuingInfo_Page.IIssuingInfo, PolicyForms_Page.IPolicyForms, FEIN_Page.IFEIN
{
    ...
    public void Regression_Scenario_Transaction_009_NBQU() throws IOException, InvalidFormatException, ParserConfigurationException, SAXException, InterruptedException
    {
        initialTransactionName = "NBQU";
        excelFileName = "GL_Regression_09";
    ...
    @Test(enabled = true,  priority = 1, dependsOnMethods = "Regression_Scenario_Transaction_009_NBQU" )
    public void Regression_Scenario_Transaction_009_PCNM() throws IOException, InvalidFormatException, ParserConfigurationException, SAXException, InterruptedException
    {
        excelFileName = "GL_Regression_09";
        excelFilePath = utils.configReader().getExcelFilePath(excelFileName);
    ...
    @Test(enabled = true, priority = 2 , dependsOnMethods = "Regression_Scenario_Transaction_009_PCNM")
    public void Regression_Scenario_Transaction_009_DNRE() throws IOException, InvalidFormatException, ParserConfigurationException, SAXException, InterruptedException
    {
        excelFileName = "GL_Regression_09";
        excelFilePath = utils.configReader().getExcelFilePath(excelFileName);
        testDataAsMap = utils.excelFileUtil().getExcelDataAsMapWithMultipleValues(excelFilePath, "DNRE_1");
        ...
        if(testDataAsMap.get("Transaction_Status").get(0).equalsIgnoreCase("$$NA$$") || testDataAsMap.get("Transaction_Status").get(0).equalsIgnoreCase("Fail"))
        {
            ...
            whileViewing().theTransactionInformationPage().theUser_().enterDNREDetails(testDataAsMap);
            whileViewing().theIssuePage().theUser_().issuePolicy();
            whileViewing().theCMPHomePage().theUser_().checkTransactionStatus(sanNumber);
            whileViewing().theCMPLoginPage().theUser_().logOut();
        }
    ...
    @Test(enabled = true, priority = 3 , dependsOnMethods = "Regression_Scenario_Transaction_009_DNRE")
    public void Regression_Scenario_Transaction_009_RDNR() throws IOException, InvalidFormatException, ParserConfigurationException, SAXException, InterruptedException
    {
        excelFileName = "GL_Regression_09";
        excelFilePath = utils.configReader().getExcelFilePath(excelFileName);
...
*/
    ...
    public void afterMethod(ITestResult testResult) throws ParserConfigurationException, InvalidFormatException, SAXException, IOException
    {
        ...
        excelFileName = "GL_Regression_08";
        excelFilePath = utils.configReader().getExcelFilePath(excelFileName);
