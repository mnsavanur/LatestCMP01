...
public class Regression_Scenario_GL_011
    ...
    public void Regression_Scenario_GL_011_Transaction_001_NBQU() throws IOException, InvalidFormatException, ParserConfigurationException, SAXException, InterruptedException
    {
        initialTransactionName = "NBQU";
        excelFileName = "GL_Regression_11";
    ...
    @Test(enabled = true, priority = 1, dependsOnMethods = "Regression_Scenario_GL_011_Transaction_001_NBQU")
    public void Regression_Scenario_GL_011_Transaction_002_CANC() throws IOException, InvalidFormatException, ParserConfigurationException, SAXException, InterruptedException {
        excelFileName = "GL_Regression_11";
        excelFilePath = utils.configReader().getExcelFilePath(excelFileName);
    ...
    @Test(enabled = true, priority = 2, dependsOnMethods = "Regression_Scenario_GL_011_Transaction_002_CANC")
    public void Regression_Scenario_GL_011_Transaction_003_VIEW() throws IOException, InvalidFormatException, ParserConfigurationException, SAXException, InterruptedException {
        excelFileName = "GL_Regression_11";
        excelFilePath = utils.configReader().getExcelFilePath(excelFileName);
        testDataAsMap = utils.excelFileUtil().getExcelDataAsMapWithMultipleValues(excelFilePath, "VIEW");
        ...
        if (testDataAsMap.get("Transaction_Status").get(0).equalsIgnoreCase("$$NA$$") || testDataAsMap.get("Transaction_Status").get(0).equalsIgnoreCase("Fail")) {
            ...
            whileViewing().theClassCodePage().theUser_().increaseExposure(testDataAsMap);
            whileViewing().theCMPLoginPage().theUser_().logOut();
        }
    }
