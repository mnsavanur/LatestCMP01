...
public class Regression_Scenario_CAP_15
        ...
        implements CMPLogin_Page.ILogin, CMPHome_Page.IHome, CommonInfo_Page.ICommonInfo, Names_Page.INames, Rating_Page.IRating, Issue_Page.IIssue_Page, IssuingInfo_Page.IIssuingInfo, PolicyForms_Page.IPolicyForms, FEIN_Page.IFEIN
{
    ...
    public void Regression_Scenario_CAP_15_Transaction_001_NBQUNBIS() throws Exception {
        initialTransactionName = "NBQUNBIS";
        excelFileName = "Regression_Scenario_CAP_15_Test_Datafile";
        ...
        if (testDataAsMap.get("Transaction_Status").get(0).equalsIgnoreCase("$$NA$$") || testDataAsMap.get("Transaction_Status").get(0).equalsIgnoreCase("Fail")) {
            ...
            whileViewing().theCapitalAssetPage().theUser_().addOptionalCovgs(testDataAsMap,"Regression_Scenario_CAP_15");
            ...
            whileViewing().theCAPLocationPage().theUser_().addLocationCovgs(testDataAsMap,"Regression_Scenario_CAP_15");
    ...
    @Test(enabled = true,  priority = 1, dependsOnMethods = "Regression_Scenario_CAP_15_Transaction_001_NBQUNBIS" )
    public void Regression_Scenario_CAP_15_Transaction_002_PCNM() throws IOException, InvalidFormatException, ParserConfigurationException, SAXException, InterruptedException
    {
        excelFileName = "Regression_Scenario_CAP_15_Test_Datafile";
        excelFilePath = utils.configReader().getExcelFilePath(excelFileName);
        ...
        if(testDataAsMap.get("Transaction_Status").get(0).equalsIgnoreCase("$$NA$$") || testDataAsMap.get("Transaction_Status").get(0).equalsIgnoreCase("Fail"))
        {
            ...
            whileViewing().theRatingPage().theUser_().ratePolicyWithoutVerification(testDataAsMap);
            whileViewing().theIssuingInfo().theUser_().CPfillIssuingInfoDetails(testDataAsMap, sanNumber, "PCNM");
            whileViewing().theBillingPage().theUser_().fillBillingPageDetails(testDataAsMap);
            whileViewing().theMiscInformationPage().theUser_().fillMiscInformation(testDataAsMap, sanNumber);
            whileViewing().thePolicyFormsPage().theUser_().fillsInPolicyForms2(testDataAsMap);
            whileViewing().theIssuePage().theUser_().issuePolicy();
    ...
    @Test(enabled = true, priority = 2, dependsOnMethods = "Regression_Scenario_CAP_15_Transaction_002_PCNM")
    public void Regression_Scenario_CAP_15_Transaction_003_NARR() throws IOException, InvalidFormatException, ParserConfigurationException, SAXException, InterruptedException {
        excelFileName = "Regression_Scenario_CAP_15_Test_Datafile";
        excelFilePath = utils.configReader().getExcelFilePath(excelFileName);
